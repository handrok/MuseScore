...
DoubleRepeatMeasure::DoubleRepeatMeasure(Score* score)
   : RepeatMeasure(score)
      {
      }

void DoubleRepeatMeasure::layout()
      {
      for (Element* e : el())
            e->layout();
      ...
      // add the second line of repeat
      const qreal shift = lw * 2;
      w += shift;
      path.moveTo(w - lw, y);
      path.lineTo(w,  y);
      path.lineTo(lw + shift,  h+y);
      path.lineTo(shift, h+y);
      path.closeSubpath();
